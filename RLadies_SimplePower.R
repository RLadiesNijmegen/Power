## power simulation code for RLadies Nijmegen
## Laurel Brehm, 06/06/19
...
## use a linear model wrapped in a one-way anova to test whether there is any difference between any groups.
...
### fully-crossed design with two repeated measures between design with a continuous outcome
...
powerFuncLMB<- function(s,i){
  ...
  ## you might have something simpler, where each person sees each item in each condition,
  ## in that case you'd use a simpler combination of rep and seq.
  ## but here, we want to make sure we rotate through as in a latin square-- what we are setting up here is like having 4 lists in your experiment.
...
## started by copying the above and made a few changes...
